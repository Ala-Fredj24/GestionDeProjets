...
@Component({
  ...
  template: `
    <section class="page" *ngIf="projet">
      ...
      <div class="card">
        <h2>Affecter des employes</h2>

        <div *ngIf="employes.length === 0">Aucun employe disponible.</div>
  ...
  styles: [
    `
      ...
      .card { background: #fff; border-radius: 16px; padding: 24px; border: 1px solid #eef2f7; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.05); }
...
export class ProjectTeamComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.employeeService.chargerTousLesEmployes().subscribe({
      ...
      error: () => {
        this.messageErreur = 'Impossible de charger les employes.';
      },
    });
  }
  ...
  enregistrer(): void {
    ...
    this.projectService.affecterEmployesAuProjet(this.projet.id, ids).subscribe({
      next: (projet) => {
        this.projet = projet;
        this.messageSucces = 'Equipe affectee avec succes.';
        this.messageErreur = '';
      },
      error: () => {
