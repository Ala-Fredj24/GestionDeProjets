        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let tache of taskService.taches()">
              ...
              <td class="actions">
                <a class="link-button" [routerLink]="['/taches', tache.id, 'modifier']">Modifier</a>
                <button type="button" class="danger-button" (click)="supprimerTache(tache)">Supprimer</button>
              </td>
            </tr>
          </tbody>
        </table>
...
export class TaskListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.taskService.chargerToutesLesTaches().subscribe({
      error: (error) => {
        this.messageErreur = `Impossible de charger les tâches. ${error?.status ?? ''} ${error?.statusText ?? ''}`.trim();
      }
    });
  }
  ...
  supprimerTache(tache: Task): void {
    ...
    this.taskService.supprimerTache(tache.id).subscribe({
      ...
      error: (error) => {
        this.messageErreur = `Erreur suppression tâche : ${error?.status ?? ''} ${error?.statusText ?? ''}`.trim();
      }
    });
  }
}
